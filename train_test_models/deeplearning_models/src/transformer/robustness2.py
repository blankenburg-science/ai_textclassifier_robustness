...
if __name__ == "__main__":
    ...
    for fun_index in [0]: #range(0,len(attack_list)):
        ...
        for sample_run in range(samples):
            for perturbation_prob in perturbation_prob_vec:
                ...
                idx = opt.fold
                df_results_original = df_results_original.append(pd.DataFrame({'Mode': [model_str], 'Perturbation Training': ['No Training'],'Dataset': [dataset_name] , 'Sample': [sample_run], 'Random_Seed': [idx], 'Perturbation': [attack_name], 'Perturbation_Strength': [perturbation_prob], 'ACC': [accuracy]}))

                suitcase = [df_results_original]
                filename = "perturbation_curve_samples_"+str(samples)+"_"+model_str+"_"+dataset_name+"_"+attack_name+"_"+str(opt.fold)
