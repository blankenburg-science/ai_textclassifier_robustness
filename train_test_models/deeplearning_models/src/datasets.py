...
class AgNews(object):
    ...
    """
    print(object)
    def __init__(self, fold, input_folder):
        print(fold)
        ...
        self.data_folder = "{}/{}/{}".format(DATA_FOLDER, self.data_name, self.input_folder)
        self.n_classes = 4
        self.epoch_size = 5000
        self.fold = fold
        # Check if relevant files are in the folder_path
        if os.path.exists(self.data_folder):
            for f in ["classes.txt", "readme.txt", "test_"+str(self.fold)+".csv", "train_"+str(self.fold)+".csv"]:
                print('------------------------------------------')
                print(os.path.join(self.data_folder, f))
                print('------------------------------------------')
                if not os.path.exists(os.path.join(self.data_folder, f)):
                    print('Not Found:'+os.path.join(self.data_folder,f))
    ...
    def load_train_data(self):
        return self._generator(os.path.join(self.data_folder, "train_"+str(self.fold)+".csv"))

    def load_test_data(self):
        return self._generator(os.path.join(self.data_folder, "test_"+str(self.fold)+".csv"))
...
class YelpPolarity(object):
    ...
    def __init__(self, fold):
        ...
        # Check if relevant files are in the folder_path
        if os.path.exists(self.data_folder):
            for f in ["readme.txt", "test_"+str(self.fold)+".csv", "train_"+str(self.fold)+".csv"]:
                if not os.path.exists(os.path.join(self.data_folder, f)):
                    self._ = get_file(self.data_name, origin=self.url, untar=True, cache_subdir=self.data_folder)
        else:
            self._ = get_file(self.data_name, origin=self.url, untar=True, cache_subdir=self.data_folder)
...
def load_datasets(names=["ag_news", "imdb"], fold=0, input_folder='raw'):
    """
    ...
    if 'ag_news' in names:
        datasets.append(AgNews(fold, input_folder))
    if 'db_pedia' in names:
        datasets.append(DbPedia(fold))
    if 'yelp_review' in names:
        datasets.append(YelpReview())
    if 'yelp_polarity' in names:
        datasets.append(YelpPolarity(fold))
